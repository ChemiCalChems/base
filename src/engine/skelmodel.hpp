...
struct skelmodel : animmodel
{
    ...
    struct skelmesh : mesh
    {
            ...
            loopi(numtris)
            {
                ...
                loopj(3)
                {
                    ...
                    loopk(htlen)
                    {
                        ...
                        else if(!memcmp(&vverts[vidx], &vv, sizeof(vv))) { minvert = std::min(minvert, idxs.add(ushort(vidx))); break; }
                    }
                }
            }
            elen = idxs.length()-eoffset;
        ...
        void setshader(Shader *s, int row)
        {
            ...
            if(row) s->setvariant(g->skel->usegpuskel ? std::min(maxweights, g->vweights) : 0, row);
            else if(g->skel->usegpuskel) s->setvariant(std::min(maxweights, g->vweights)-1, 0);
            else s->set();
        }
    ...
    struct skeleton
    {
        ...
        {
            loopi(numbones) if(cubematchstr(bones[i].name, name)) return i;
            return -1;
        }
        ...
        int availgpubones() const { return std::min(maxvsuniforms, maxskelanimdata) / 2; }
...
template<class MDL> struct skelcommands : modelcommands<MDL, struct MDL::skelmesh>
{
    ...
    static void setpitch(char *name, float *pitchscale, float *pitchoffset, float *pitchmin, float *pitchmax)
    {
        ...
        {
            vector<int> elems;
            if(((meshgroup *)mdl.meshes)->skel->findbones(name, elems))
            {
                loopv(elems)
                {
                    boneinfo &b = ((meshgroup *)mdl.meshes)->skel->bones[elems[i]];
    ...
    static void setpitchtarget(char *name, char *animfile, int *frameoffset, float *pitchmin, float *pitchmax)
    {
        ...
        {
            vector<int> elems;
            if(skel->findbones(name, elems))
            {
                loopv(elems)
                {
                    bool skip = false;
                    loopvj(skel->pitchtargets) if(skel->pitchtargets[j].bone == elems[i]) { skip = true; break; }
                    if(skip) continue;
    ...
    static void setanimpart(char *maskstr)
    {
        ...
        loopv(bonestrs)
        {
            ...
            {
                vector<int> elems;
                if(((meshgroup *)p->meshes)->skel->findbones(bonestr[0]=='!' ? bonestr+1 : bonestr, elems)) loopv(elems)
                {
                    bonemask.add(elems[i] | (bonestr[0]=='!' ? BONEMASK_NOT : 0));
                    num++;
                }
            }
    ...
    static void setadjust(char *name, float *yaw, float *pitch, float *roll, float *tx, float *ty, float *tz)
    {
        ...
        if(mdl.meshes)
        {
            ...
            if(((meshgroup *)mdl.meshes)->skel->findbones(name, elems))
            {
                loopv(elems)
                {
                    while(!MDL::adjustments.inrange(elems[i])) MDL::adjustments.add(skeladjustment(0, 0, 0, vec(0, 0, 0)));
                    MDL::adjustments[elems[i]] = skeladjustment(*yaw, *pitch, *roll, vec(*tx/4, *ty/4, *tz/4));
                }
                return;
            }
        }
